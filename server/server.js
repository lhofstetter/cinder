 ...
 */
app.put("/listing/:listing_id", validateListingId, async (req, res) => {
    try {
        ...
        const { images_to_remove } = req.body;
        const deleteQueue = [];
        ...
        const listingFormData = req.body;
        await db
            .update(listings)
            .set({
 ...
 */
app.delete("/listing/:listing_id", validateListingId, async (req, res) => {
    let { listing_id: listing_id_string } = req.params;
    const listing_id = parseInt(listing_id_string);
    try {
        const deleteQueue = [];
 ...
 * @returns {ListingData}
 */
app.get("/listing/:listing_id", validateListingId, async (req, res) => {
    console.log("hello");
    let { listing_id: listing_id_string } = req.params;
    const listing_id = parseInt(listing_id_string);
    try {
        ...
        const listingTags = listingTagData.map((tagData) => tagData.tag_name);
        const listingImageData = await db
            .select({ source: images.source })
            .from(images)
            .where(eq(images.listing_id, listing_id));
        const listingImages = listingImageData.map((imageData) => imageData.source);
        const responseData = {
            ...listingFromDb,
            tags: listingTags,
