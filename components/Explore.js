...
const Advanced = () => {
  ...
  const getListings = async () => {
    ...
    for (i = 0; i < validSizes.Shoes.length; i++) {
      if (i >= min && i <= max) {
        if (!shoe.includes(validSizes.Shoes[i])) shoe.push(validSizes.Shoes[i]);
      } else {
    ...
    for (i = 0; i < validSizes.Tops.length; i++) {
      if (i >= min && i <= max) {
        if (!top.includes(validSizes.Tops[i])) top.push(validSizes.Tops[i]);
      } else {
    ...
    for (i = 0; i < validSizes.Bottoms.length; i++) {
      if (i >= min && i <= max) {
        if (!bottom.includes(validSizes.Bottoms[i])) bottom.push(validSizes.Bottoms[i]);
      } else {
    ...
    for (i = 0; i < validSizes.Accessories.length; i++) {
      if (i >= min && i <= max) {
        if (!accessory.includes(validSizes.Accessories[i])) accessory.push(validSizes.Accessories[i]);
      } else {
  ...
  const swiped = async (direction, nameToDelete, id) => {
    ...
    if (direction == "right") {
      ...
      if (data?.matches) {
        ...
        Alert.alert(
          `You matched with the owner of "${listingData.listing_name}"! Shoot them a message and coordinate a swap!"`,
        );
      }
    } else {
  ...
  return (
    <View style={exploreStyles.container}>
      ...
      <View>
        <Modal visible={settings.visible} transparent={true} animationType="slide" ref={modalRef}>
          <View
            style={{
            ...
            <View style={{display: "flex", flexDirection: "column", marginLeft: "5%"}}>
              {selectedCategories["Tops"][0] ? (
                <View style={{display: "flex", flexDirection: "row"}}>
                  ...
                  <TextInput
                    value={topSizes[0]}
                    onChangeText={handleMinChangeSizes}
                    autoCapitalize="characters"
                    ref={minTopSizeRef}
                    onSubmitEditing={() => {
                      if (validSizes.Tops.includes(topSizes[0])) minTopSizeRef.current.focus();
                      else
                        Alert.alert("Invalid size", "Please enter a valid minimum size!", [
                          {
                            text: "Ok",
                            onPress: () => {
                    ...
                    enterKeyHint="next"
                    style={{marginTop: "5%", borderWidth: 1, padding: 5, borderRadius: 5}}
                    maxLength={3}
                  />
                  <Text style={{fontFamily: "Inter", marginTop: "5%"}}> - </Text>
                  <TextInput
                    value={topSizes[1]}
                    onChangeText={handleMaxChangeSizes}
                    autoCapitalize="characters"
                    ref={maxSizeRef}
                    style={{marginTop: "5%", borderWidth: 1, padding: 5, borderRadius: 5}}
                    enterKeyHint="done"
                    ...
                    onEndEditing={() => {
                      if (!validSizes.Tops.includes(topSizes[1]))
                        Alert.alert("Invalid size", "Please enter a valid maximum size!", [
                          {
                            text: "Ok",
                            onPress: () => {
              ...
              {selectedCategories["Bottoms"][0] ? (
                <View style={{display: "flex", flexDirection: "row"}}>
                  ...
                  <TextInput
                    value={bottomSizes[1]}
                    onChangeText={handleMaxChangeSizesBottoms}
                    autoCapitalize="characters"
                    ref={maxSizeRefBot}
                    style={{marginTop: "5%", borderWidth: 1, padding: 5, borderRadius: 5}}
                    enterKeyHint="done"
              ...
              {selectedCategories.Shoes[0] ? (
                <View style={{display: "flex", flexDirection: "row"}}>
                  ...
                  <TextInput
                    value={shoeSizes[1]}
                    onChangeText={handleMaxChangeShoesSizes}
                    autoCapitalize="characters"
                    ref={maxShoeSizeRef}
                    style={{marginTop: "5%", borderWidth: 1, padding: 5, borderRadius: 5}}
                    enterKeyHint="done"
              ...
              {selectedCategories.Accessories[0] ? (
                <View style={{display: "flex", flexDirection: "row"}}>
                  <Text style={{fontFamily: "Inter", marginTop: "7%", fontSize: 15}}>Accessory Sizes: </Text>
                  <TextInput
                    value={accessorySizes[0]}
                    onChangeText={handleMinChangeAccessorySizes}
                    ref={minAccessorySizeRef}
                    autoCapitalize="characters"
                    onSubmitEditing={() => {
                  ...
                  <Text style={{fontFamily: "Inter", marginTop: "5%"}}> - </Text>
                  <TextInput
                    value={accessorySizes[1]}
                    onChangeText={handleMaxChangeAccessorySizes}
                    autoCapitalize="characters"
                    ref={maxAccessorySizeRef}
                    style={{marginTop: "5%", borderWidth: 1, padding: 5, borderRadius: 5}}
                    enterKeyHint="done"
                    maxLength={3}
                    onEndEditing={() => {
            ...
            <Pressable
              onPress={() => {
                if (
                  validSizes["Bottoms"].includes(bottomSizes[0]) &&
                  validSizes["Bottoms"].includes(bottomSizes[1]) &&
                  validSizes["Tops"].includes(topSizes[0]) &&
                  validSizes["Tops"].includes(topSizes[1]) &&
                  validSizes.Shoes.includes(shoeSizes[0]) &&
                  validSizes.Shoes.includes(shoeSizes[1]) &&
                  validSizes.Accessories.includes(accessorySizes[0]) &&
                  validSizes.Accessories.includes(accessorySizes[1])
                )
                  setSettings({visible: false});
                else {
                  Alert.alert(
                    "Invalid settings :(",
                    "It looks like one of the sizes you entered isn't valid. Please double-check your filters and try again.",
                  );
                }
              }}
              style={{
