...
export default function AccountCreate() {
  ...
  const handleSignUp = async function () {
   ...
   let form = new FormData();
	const renderedImage = ImageManipulator.ImageManipulator.manipulate(itemImage).renderAsync(); // this is wasteful and needs to be rewritten
	const result = await renderedImage.saveAsync({
		format: ImageManipulator.SaveFormat.WEBP,
	});

		
    form.append("file", {uri: result.uri, type: "image/webp", name: itemImage.fileName});
  ...
  const pickImage = async () => {
    ...
    result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.useCameraPermissions.Images,
      quality: 1,
    });
    if (!result.canceled) {
  ...
  const handlePhoneNumberInput = text => {
    if (text.length === 10) /** @todo support non-US phone numbers */ classOfRef.current.focus();
    setPhoneNumber(text.replace(/\D/g, ""));
  };

  const handleYearInput = text => {
    if (text.length === 4) bioRef.current.focus();
